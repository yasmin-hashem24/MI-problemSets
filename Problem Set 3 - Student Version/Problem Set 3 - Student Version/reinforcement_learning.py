...
class RLAgent(Agent[S, A]):
    ...
    def act(self, env: Environment[S, A], observation: S, training: bool = False) -> A:
        # Get the list of available actions for the given environment
        actions = env.actions()

        if training and self.should_explore():
            # If the agent is in training mode and should explore, select a random action
            # TODO: Return a random action whose index is "self.rng.int(0, len(actions)-1)"
            random_action_index = self.rng.int(0, len(actions) - 1)
            return actions[random_action_index]
        else:
            # If the agent is not in training mode or should not explore, select the action
            # with the highest Q-value for the given observation
            q_value_actions = [
                (self.compute_q(env, observation, action), action) for action in actions]
            # Sort the actions by their Q-value in descending order
            q_value_actions.sort(key=lambda item: -item[0])
            # Return the action with the highest Q-value
            return q_value_actions[0][1]
...
class SARSALearningAgent(RLAgent[S, A]):
    ...
    def update(self, env: Environment[S, A], state: S, action: A, reward: float, next_state: S, next_action: Optional[A]):
        ...
        target = reward + g * next_q
       # update Qstateaction by srsa rule
        self.Q[state][action] = q + a * (target - q)
...
class QLearningAgent(RLAgent[S, A]):
    ...
    def update(self, env: Environment[S, A], state: S, action: A, reward: float, next_state: S, done: bool):
        ...
        next_q = self.compute_utility(env, next_state) if not done else 0

        a = self.learning_rate
        g = self.discount_factor
        q = self.Q[state][action]
        target = reward + g * next_q
       # update Qstateaction by qlearning rule
        self.Q[state][action] = q + a * (target - q)
...
class ApproximateQLearningAgent(RLAgent[S, A]):
    ...
    def __compute_q_from_features(self, features: Dict[str, float], action: A) -> float:
        ...
        return sum([self.weights[action][feature_name] * feature_value for feature_name, feature_value in features.items()])
    ...
    def update(self, env: Environment[S, A], state: S, action: A, reward: float, next_state: S, done: bool):
        ...
        target = reward + g * next_q
        # update Qstateaction by qlearnapp rule
        for feature_name, feature_value in self.feature_extractor.extract_features(env, state).items():
            self.weights[action][feature_name] = self.weights[
                action][feature_name] + a * (target - q) * feature_value
