...
def solve_via_human(problem: SudokuProblem):
    ...
    while True:
        ...
        if variable not in problem.variables:
            print(
                f"Unknown variable. Valid variables are: {problem.variables}")
            continue
        if value not in problem.domains[variable]:
            print(
                f"Invalid value. The domain of the variable {variable} is {problem.domains[variable]}")
            continue
    ...
    if result is None:
        ...
        if problem.is_complete(result):
            if problem.satisfies_constraints(result):
                ...
                print(
                    "The result is a complete assignment, but it does not satisfy all the constraints.")
        else:
